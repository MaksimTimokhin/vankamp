...
public:
    ...
    VanKampenDiagram GenerateRandomDiagram(size_t steps = 5) {
        std::uniform_int_distribution<size_t> relation_distribution(0, group_.RelationCount() - 1);
        ...
        for (size_t step = 0; step < steps; ++step) {
            auto next_relation = group_.GetRelationByIndex(relation_distribution(generator_));
            ...
            std::uniform_int_distribution<size_t> element_distribution(0, word.size() - 1);
            ...
            if (it == next_relation.end()) {
                ...
                it = std::find(next_relation.begin() + random_relation_index, next_relation.end(),
                               word[random_word_index]);
                if (it == next_relation.end()) {
                    --step;
                    continue;
                }
            ...
            for (; it + equality_len < next_relation.end() &&
                 ...
                 ++equality_len) {
                if (*(it + equality_len) != word[random_word_index + equality_len - 1]) {
                    break;
                }
            }

            std::vector<int> new_path;
            std::copy(it + equality_len, next_relation.end(), std::back_inserter(new_path));
            ...
            word.insert(word.begin() + random_word_index, new_path.begin(), new_path.end());

            std::vector<size_t> new_indices;
            for (int index = diagram.Size(); index < diagram.Size() + new_path.size() - 1;
                 ++index) {
                new_indices.push_back(index);
            }

            word_graph_indices.erase(word_graph_indices.begin() + random_word_index + 1,
                                     word_graph_indices.begin() + random_word_index + equality_len);
            word_graph_indices.insert(word_graph_indices.begin() + random_word_index + 1,
                                      new_indices.begin(), new_indices.end());

            diagram.Resize(diagram.Size() + new_path.size() - 1);
            for (int i = 0; i < new_path.size(); ++i) {
                std::string label = group_.GetVariableNameByIndex(word[random_word_index + i]);
